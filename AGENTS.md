...
图上它和 SID/STAR 一样画虚线：要表达的区别只有「按图走 vs 沿航路飞」这一条，三
种程序各给一种线型，读的人得先学会一套图例。
...
因此 `RouteGenerator` 里改写后的串存在**另一个 ref**，不就地改 `plan.route`：就
地改掉的话第二次改写会拿上一次的结果当原始值，于是换第二条 SID 时删不掉第一条。
