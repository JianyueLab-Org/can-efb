...
的 `applySelection`：选了哪一项就替换 `aip/resolve` 展开的那一段；没选的 SID/STAR
按航路串里的名字从机场数据找回来，只画实际飞的那几段（resolve 展开的是整串腿，所有
跑道转换首尾相接）。机场数据没取到时保持 resolve 的原样。
...
- 进近的腿带 `part` 时按 part 分段：`transition` 是可选的转换，`missed` 是复飞段。
  ...
  NAIP 有 37 条进近把复飞的头几条腿切进了 `final`：final 里从第一条爬升/航向腿
  （`CA`/`VA`/`VI`/`VM`/`FA`/`CI`）起归复飞段，复飞段已走过它们的定位点时丢掉那份副本。
- 合成时坐标完全相同的相邻两点算一个点（留先到的）：复飞点常常就在跑道头上，两个名字
  各出一个标注会叠在一起。
...
- 腿的转弯方向记在**这条腿的起点**上（ARINC 424：转上这条腿的那个弯）。`RF` 和等待
  腿的方向不往前记。
...
- 照航图画：两条边各一支方向箭头（`hold-arrows`，z9 起），实心等腰三角形压在线上、
  和线同色，放在从这条边起点往里三成处，转弯弧上不放。入航磁航向只写一次，在跑道形正
  中（`hold-courses`，z10 起，14 号字，不随边转，参与避让），不写出航的反方向，也不从
  真方位倒算。箭头是 SDF 图（`hold-arrow`），不用字形 —— 本站只带 Noto Sans 0–511 两
  片。**SDF 图要写真正的距离场**（alpha 0.75 是边，每 1/8 一个 CSS 像素）：画一个实心
  形状标成 SDF，本体被削掉一圈、描边没地方画，第一版因此在地图上只剩三四个像素。
