...
const level = ref("");
/*
 * 「不使用受限汇编」以前是这里的一个勾选框，只管航路生成。现在是全站一个开关，在
 * 设置页，所有 can-db 请求一起跟着走（lib/naip.ts）—— 只排掉航路里的 NAIP、图上却
 * 照样画着它，给出去的就是一份自相矛盾的东西。
 */
    ...
    <template v-if="plan">
      ...
      <p class="text-xs text-muted">
        ...
        <!--
          ...
          值取 can-db 报的 `plan.unrestricted`，不是设置里那个开关：答案该由产出它的那一
