 ...
 *
 * **浏览器从不直连它**：要么是服务端渲染时用（`server/canDb.ts`），要么是岛屿
 * 经本站的 `/api/db/*` 反代去取 —— 地图上的航路、跑道、机场地面、进离场程序都
 * 走那条路，白名单和每条的调用方写在 `pages/api/db/[...path].ts` 里。这样不用给
 * can-db 开 CORS，cookie 也由本站同源带上。新的浏览器端用途在那张白名单上逐条
 * 加，别开通配。
 *
