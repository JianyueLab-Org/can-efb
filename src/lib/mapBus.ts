...
export interface MapPayload {
  /**
   * **连成线**的点 —— 一条航路。相邻两点之间画大圆弧，样式按 SID/STAR 分。
   *
   * 空数组或省略＝没有航路。
   */
  points?: MapPoint[];
  /**
   * **只画点、不连线**的点 —— 一批彼此无关的位置，比如全国的机场。
   *
   * 和 `points` 分成两个字段而不是加一个 `connect: boolean`：一次推送里两者可以
   * 同时存在（航路画在机场底图上），一个布尔量表达不了。而把机场塞进 `points`
   * 的后果不是"样式不对"，是**几百个机场被顺次连成一条面条**，那正是这个字段存
   * 在的理由。
   */
  markers?: MapPoint[];
  /**
   * 有就把视野对到它，而不是框住全部。
   *
   * 用途是"在一堆点里挑一个看"——机场列表点一行，地图该飞过去，而不是重新框住全
   * 国。没有它就只能靠重推 markers 来改变视野，那会把整层重画一遍。
   */
  focus?: MapPoint;
